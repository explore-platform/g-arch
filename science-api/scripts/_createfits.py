def CreateFITS(gaia_fits='spectra.fits', gaia_csv='GAIA_ID.csv', target_csv='test_id.csv', resolve=False, output_base=''):
    ...
    np.savetxt(output_base+'result_xmatch.dat', np.array([tab['index_1'],tab['source_id']  ] ).T, delimiter='\t', fmt=" %i  %i")
    ...
    if len(tab)==0:
        ...
        gaia_xm =  np.genfromtxt(output_base+'result_xmatch.dat')
    ...
    hdul.writeto(output_base+'spectra_xmatch.fits', overwrite=True );
    hdul.close()
    del(hdul, hdu1, hdu2)
    print("Created: "+output_base+'spectra_xmatch.fits'+" (found "+str(len(tab))+" spectra)")
    return
